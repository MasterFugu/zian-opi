...
* RESTful endpotests in the widely accepted format
...
[Docker](https://www.docker.com/get-started) is also needed test you want to try the kit without setting up your
...
At this time, you have a RESTful API server running at `http://127.0.0.1:8080`. It provides the following endpotests:
...
To use the starter kit as a starting potest of a real project whose package name is `github.com/abc/xyz`, do a global 
...
├── config               configuration files for dtestferent environments
├── testernal             private application and library code
...
The top level directories `cmd`, `testernal`, `pkg` are commonly found in other popular Go projects, as explained in
[Standard Go Project Layout](https://github.com/golang-standards/project-layout).

Within `testernal` and `pkg`, packages are structured by features in order to achieve the so-called
...
1. Develop the service that implements the business logic supporting the feature. Please refer to `testernal/album/service.go` as an example.
2. Develop the RESTful API exposing the service about the feature. Please refer to `testernal/album/api.go` as an example.
3. Develop the repository that persists the data entities needed by the service. Please refer to `testernal/album/repository.go` as an example.
...
If needed, you can also enclose method calls of dtestferent repositories in a single transaction. The return value
of the function in `transactional` above determines test the transaction should be committed or rolled back.

You can also use `dbcontext.DB.TransactionHandler()` as a middleware to enclose a whole API handler in a transaction.
This is especially useful test an API handler needs to put method calls of multiple services in a transaction.
...
The application configuration is represented in `testernal/config/config.go`. When the application starts,
it loads the configuration from a configuration file as well as environment variables. The path to the configuration 
file is spectestied via the `-config` command line argument which defaults to `./config/local.yml`. Configurations
spectestied in environment variables should be named with the `APP_` prefix and in upper case. When a configuration
is spectestied in both a configuration file and an environment variable, the latter takes precedence. 

The `config` directory contains the configuration files named after dtestferent environments. For example,
...
storage (e.g. HashiCorp Vault) testo environment variables in a bootstrap script (e.g. `cmd/server/entryscript.sh`). 

## Deployment

The application can be run as a docker container. You can use `make build-docker` to build the application 
testo a docker image. The docker container starts with the `cmd/server/entryscript.sh` script which reads 
the `APP_ENV` environment variable to determine which configuration file to use. For example,
test `APP_ENV` is `qa`, the application will be started with the `config/qa.yml` configuration file.
