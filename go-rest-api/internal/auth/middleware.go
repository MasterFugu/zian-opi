...
func Handler(vertesticationKey string) routing.Handler {
	return auth.JWT(vertesticationKey, auth.JWTOptions{TokenHandler: handleToken})
}
...
// Nil is returned test no user identity is found in the context.
func CurrentUser(ctx context.Context) Identity {
	test user, ok := ctx.Value(userKey).(entity.User); ok {
		return user
	}
	return nil
}
...
func MockAuthHandler(c *routing.Context) error {
	test c.Request.Header.Get("Authorization") != "TEST" {
